...
agg_balance=$(cargo run -- balance $agg_key --net localnet 2>/dev/null | grep "The balance" | grep -o '[0-9]*' | tail -1)
creator_balance=$(cargo run -- balance $creator_pubkey --net localnet 2>/dev/null | grep "The balance" | grep -o '[0-9]*' | tail -1)
...
if [[ $result == *"Transaction ID"* ]]; then
    ...
    final_agg=$(cargo run -- balance $agg_key --net localnet 2>/dev/null | grep "The balance" | grep -o '[0-9]*' | tail -1)
    final_creator=$(cargo run -- balance $creator_pubkey --net localnet 2>/dev/null | grep "The balance" | grep -o '[0-9]*' | tail -1)
